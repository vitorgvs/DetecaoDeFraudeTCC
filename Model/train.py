...
Lê a ABT (Dados/abt.csv), separa treino/teste/validação, executa otimização Optuna
maximizando F-beta (beta=2), treina o modelo final, executa Cross-Validation
estratificada para detectar overfitting, e salva o modelo treinado.

Uso:
    python Model/train.py [--trials N] [--timeout SEC] [--no-optuna]
"""
...
# ---------------------------------------------------------------------------
# Optuna Objective - Maximizar F-beta (beta=2)
# ---------------------------------------------------------------------------
def create_optuna_objective(X_train, y_train, X_val, y_val):
    """Cria função objective para Optuna otimizando F-beta (beta=2)."""
    ...
    def objective(trial):
        ...
        # Treino com early stopping (XGBoost >= 2.0 usa set_params)
...
def run_optuna(X_train, y_train, X_val, y_val, n_trials=30, timeout=600):
    """Executa otimização Optuna."""
    if not OPTUNA_AVAILABLE:
        logger.warning("Optuna não disponível, usando hiperparâmetros padrão")
        return {}

    logger.info("=" * 60)
    logger.info("OTIMIZAÇÃO OPTUNA - Maximizando F-beta (beta=2)")
    logger.info("=" * 60)
...
# Cross-Validation
# ---------------------------------------------------------------------------
def run_cross_validation(X_trainval: pd.DataFrame, y_trainval: pd.Series, best_params: dict) -> dict:
    """Executa Cross-Validation estratificada no conjunto treino+validação."""
    logger.info("=" * 60)
    logger.info("CROSS-VALIDATION (StratifiedKFold)")
    logger.info("=" * 60)
...
# ---------------------------------------------------------------------------
def train_final_model(X_trainval: pd.DataFrame, y_trainval: pd.Series, best_params: dict) -> XGBClassifier:
    """Treina modelo final com melhores parâmetros em todo o conjunto treino+validação."""
    params = HYPERPARAMS.copy()
    params.update(best_params)
    params["random_state"] = params.get("random_state", TRAIN_CFG["random_state"])

    model = XGBClassifier(**params)

    logger.info("Treinando modelo final em todo o conjunto treino+validação...")
    model.fit(X_trainval, y_trainval, verbose=True)
...
def save_model(model: XGBClassifier, metrics: dict, cv_results: dict, overfitting_report: dict, best_params: dict) -> None:
...
def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("MODEL TRAINING - Fraud Detection (Optuna F-beta + CV)")
    logger.info("=" * 60)
    logger.info("Target: Maximizar F-beta (beta=2) via Optuna")
    ...
    # Combinar treino + validação para CV e treino final
    X_trainval = pd.concat([X_train, X_val], axis=0)
    y_trainval = pd.concat([y_train, y_val], axis=0)

    # 3. Optuna Optimization
    if not args.no_optuna and OPTUNA_AVAILABLE:
        best_params = run_optuna(X_train, y_train, X_val, y_val, args.trials, args.timeout)
    else:
        logger.info("Usando hiperparâmetros padrão do model_config.json")
        best_params = {}

    # 4. Cross-Validation
    cv_results = run_cross_validation(X_trainval, y_trainval, best_params)

    # 5. Treino final
    logger.info("=" * 60)
    logger.info("TREINAMENTO FINAL (treino + validação)")
    logger.info("=" * 60)
    ...
    # 8. Salvar
    save_model(final_model, test_metrics, cv_results, overfitting_report, best_params)
